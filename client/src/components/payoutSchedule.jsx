      ...
      <div className="payout-popup">
        ...
        <div className="payout-instruction-list">
          {[
            ...
            "You can update, change, or reorder anytime manual schedule is never locked.",
            "Click Save Schedule when done. Changes take effect immediately.",
            "You can also reset and start fresh at any time.",
          ].map((instruction, i) => (
...
// ── Main Component ─────────────────────────────────
export default function PayoutSchedule({ committeeId, members }) {
  ...
  useEffect(() => {
      const scheduleRequest = Promise.resolve().then(() => loadSchedule());
      return () => { void scheduleRequest; };
    }, [loadSchedule]);
  ...
  async function handleSaveManual() {
    const orders = members.reduce((result, member, index) => ({
      ...result,
      [member.id]: manualOrders[member.id] ?? index + 1,
    }), {});

    // Validate unique orders
    const values = Object.values(orders).map(Number);
    ...
    setError("");
    try {
      const memberOrders = Object.entries(orders).map(([memberId, payoutOrder]) => ({
        memberId,
    ...
    <>
      ...
      {showInstructions && (
        <ManualInstructionPopup
          ...
          onProceed={() => { setShowInstructions(false); setScheduleType("manual"); }}
        />
      )}
      ...
      <div className="payout-card">
        ...
        {!scheduleType && (
          <div className="payout-choice-grid">
            ...
            >
              <span className="payout-choice-icon">✏️</span>
        ...
        {scheduleType === "auto" && schedule.length > 0 && (
          <div className="overflow-x-auto">
            ...
            <div className="px-5 py-3 bg-[#E8EEF4] flex items-center gap-2">
              <Lock size={13} className="text-[#1E3A5F]" />
              <p className="text-xs text-[#1E3A5F]">
                This schedule was AI-generated and is permanently locked. It cannot be changed.
              </p>
            </div>
          </div>
        )}
          ...
          <>
            <div className="overflow-x-auto">
              <table className="payout-table">
                ...
                <tbody>
                  {members.map((m) => {
                    const currentOrder = manualOrders[m.id] ?? members.indexOf(m) + 1;
                    // Check for duplicate
