// // new version with better error handling and logging
...
// } = require("../services/scamDetectionService");
...
// // - Not contain "my name is", "mera naam", "naam hai" etc.
// // - Not be generic filler words
// // ─────────────────────────────────────────────────────
...
//   // Reject if it contains these phrases (user said full sentence, not just name)
...
//   // Reject if too long (a real name is rarely more than 5 words)
//   const words = lower.trim().split(/\s+/);
//   if (words.length > 5) return false;

//   // Reject if it looks like a sentence (has punctuation like ? or !)
//   if (/[?!]/.test(text)) return false;

//   return true;
// }

// // Extract just the name if user said "My name is Ali" → "Ali"
...
//       // Only return if extracted part is a reasonable name
//       if (extracted.split(/\s+/).length <= 4 && extracted.length >= 2) {
//         return extracted;
//       }
//     }
//   }
...
// async function reply(fromNumber, text, isVoice) {
//   if (isVoice) {
//     const audioPath = await convertToSpeech(text);
//     await sendVoiceMessage(fromNumber, audioPath);
//   } else {
...
//       const filePath = await downloadWhatsAppMedia(message.audio.id);
//       transcript = await transcribeAudio(filePath);
//       console.log("Transcribed text:", transcript);

//       if (!transcript) {
//         await reply(fromNumber,"معاف کیجیے، میں سمجھ نہیں پایا۔ دوبارہ کوشش کریں۔", isVoiceMessage);
//         return res.sendStatus(200);
//       }

//     } else if (message.type === "text") {
//       console.log(`Text message received from ${fromNumber}`);
...
//     // ─────────────────────────────────────────────
//     // 🛡️ SCAM SHIELD — scan EVERY message
//     // Runs before any session/intent processing.
//     // ─────────────────────────────────────────────

//     const memberships = await getMembershipsByPhone(fromNumber);
//     const isRegistered = memberships && memberships.length > 0;

//     // Only scan if the sender is a registered member
//     if (isRegistered && memberships[0]) {
//       const membership = memberships[0];

//       try {
//         // Get organizer phone for comparison
//         const { data: orgData } = await supabase
//           .from("organizers")
//           .select("phone")
//           .eq("id", membership.committees?.organizer_id)
//           .maybeSingle();

//         const scamResult = await scanMessage({
//           transcript,
//           senderPhone: fromNumber,
//           senderName: membership.name,
//           committeeId: membership.committee_id,
//           committeeName: membership.committees?.name || "Unknown",
//           organizerPhone: orgData?.phone || "",
//         });

//         if (scamResult.is_scam) {
//           // 1. Send warning to the member
//           const warningMsg = getScamWarningMessage(
//             scamResult.flags,
//             scamResult.scam_type
//           );
//           await reply(fromNumber, warningMsg, isVoiceMessage);

//           // 2. Alert the organizer (non-blocking)
//           if (orgData?.phone) {
//             const alertMsg = getOrganizerAlertMessage(
//               membership.name,
//               fromNumber,
//               transcript,
//               scamResult.risk_score,
//               scamResult.scam_type
//             );
//             sendWhatsAppMessage(orgData.phone, alertMsg).catch(() => {});
//           }

//           console.log(`🛡️ Scam detected: score=${scamResult.risk_score}, type=${scamResult.scam_type}`);
//           return res.sendStatus(200);
//         }
//       } catch (scamErr) {
//         console.error("Scam shield error (non-blocking):", scamErr.message);
//         // Don't block the message flow if scam detection fails
//       }
//     }

//     // ─────────────────────────────────────────────
//     // CHECK IF ALREADY A REGISTERED MEMBER (first)
//     // This prevents registered members from being
//     // asked for their committee code again.
//     // ─────────────────────────────────────────────

//     // isRegisteredFinal is used below instead of isRegistered
//     // (isRegistered was already computed above for the scam shield)

//     // ─────────────────────────────────────────────
//     // SESSION STATE CHECK (for mid-registration flow)
//     // ─────────────────────────────────────────────
...
//       // Try to extract name if user said full sentence
//       let finalName = extractName(rawInput) || rawInput;

//       // Validate the name
//       if (!isValidName(finalName)) {
//         await reply(
//           fromNumber,
//           "معاف کیجیے، مجھے صرف آپ کا نام چاہیے۔ جیسے: علی احمد یا سارہ خان۔ دوبارہ بتائیں؟",
//           isVoiceMessage
//         );
//         return res.sendStatus(200);
//       }
...
//         .eq("phone", fromNumber)
//         .eq("committee_id", session.pending_committee_id);
...
//       await reply(
//         fromNumber,
//        `شکریہ ${finalName}! آپ "${committee?.name}" کمیٹی میں رجسٹر ہو گئے ہیں۔ ماہانہ رقم ${committee?.monthly_amount} روپے ہے۔ مجھے بتائیں، میں آپ کی کیا مدد کر سکتا ہوں؟`,
//         isVoiceMessage
//       );
...
//     // ─────────────────────────────────────────────
//     // STATE: awaiting_payout_response
//     // Member told their payout number — asked "satisfied?"
//     // ─────────────────────────────────────────────

//     if (session?.state === "awaiting_payout_response") {
//       const lower = transcript.toLowerCase().trim();

//       // Positive responses: Yes, haan, theek hai, ok,满意, etc.
//       const positivePatterns = ["yes", "haan", "theek hai", "ok", "okay", "satisfied", "chalay", "fit", "theek"];
//       // Negative / change responses
//       const changePatterns = ["change", "badlo", "tabdeel", "change number", "number change"];

//       const isSatisfied = positivePatterns.some((p) => lower.includes(p));
//       const wantsChange = changePatterns.some((p) => lower.includes(p));

//       if (isSatisfied) {
//         // Mark position as satisfied
//         await markPositionSatisfied(session.pending_position_id);
//         await clearSession(fromNumber);

//         await reply(
//           fromNumber,
//           `بہت اچھا! آپ کی پوزیشن ${session.pending_position_number} محفوظ ہو گئی۔ شکریہ!`,
//           isVoiceMessage
//         );
//         return res.sendStatus(200);
//       }
...
//       if (wantsChange) {
//         // Move to reason collection
//         await upsertSession(fromNumber, {
//           state: "awaiting_change_reason",
//           pending_committee_id: session.pending_committee_id,
//           pending_order_id: session.pending_order_id,
//           pending_position_id: session.pending_position_id,
//           pending_position_number: session.pending_position_number,
//         });

//         await reply(
//           fromNumber,
//           `براہ کرم بتائیں کہ آپ اپنا نمبر کیوں بدلا چاہتے ہیں؟\nاپنی وجہ بتائیں — جیسے: "مجھے جلد ادائیگی چاہیے" یا "میرا میڈیکل ایمرجنسی ہے"`,
//           isVoiceMessage
//         );
//         return res.sendStatus(200);
//       }
...
//       await reply(
//         fromNumber,
//         `براہ کرم صاف صاف بتائیں:\n\n*Yes* — اگر آپ اپنے نمبر سے م满意 ہیں\n*Change* — اگر آپ نمبر بدلا چاہتے ہیں`,
//         isVoiceMessage
//       );
//       return res.sendStatus(200);
//     }
...
//     // ─────────────────────────────────────────────
//     // STATE: awaiting_change_reason
//     // Member providing reason for change request
//     // ─────────────────────────────────────────────

//     if (session?.state === "awaiting_change_reason") {
//       const reason = transcript.trim();

//       if (reason.length < 3) {
//         await reply(
//           fromNumber,
//           "براہ کرم مکمل وجہ بتائیں۔ کم از کم 3 الفاظ میں لکھیں۔",
//           isVoiceMessage
//         );
//         return res.sendStatus(200);
//       }

//       // Get all positions in this order for AI context
//       const order = await getActiveOrder(session.pending_committee_id);
//       const allPositions = order?.payout_positions || [];

//       // Create change request (AI analyzes the reason)
//       const changeRequest = await createChangeRequest({
//         payoutPositionId: session.pending_position_id,
//         memberId: session.pending_member_id,
//         reason,
//         currentPosition: session.pending_position_number,
//         totalMembers: allPositions.length,
//         allPositions,
//       });

//       // Mark position as change_requested
...
//       await reply(
//         fromNumber,
//         `آپ کی درخواست ریکارڈ ہو گئی ہے۔\n\n` +
//         `وجہ: ${reason}\n` +
//         `AI تجزیہ: ${changeRequest.ai_reasoning}\n\n` +
//         `آپ کا نمبر: ${session.pending_position_number}\n` +
//         `تجویز کردہ نیا نمبر: ${changeRequest.ai_suggested_position}\n\n` +
//         `آرگنائزر جلد ہی فیصلہ کریں گے۔ آپ کو واٹس ایپ پر اطلاع دی جائے گی۔`,
//         isVoiceMessage
//       );
//       return res.sendStatus(200);
//     }
...
//     // ─────────────────────────────────────────────
//     // STATE: awaiting_priority_voice
//     // Member sending voice note for priority payout request
//     // ─────────────────────────────────────────────

//     if (session?.state === "awaiting_priority_voice") {
//       const transcript_text = transcript.trim();

//       if (transcript_text.length < 5) {
//         await reply(
//           fromNumber,
//           "براہ کرم اپنی درخواست کی وجہ واضح طور پر بتائیں۔ کم از کم 5 الفاظ میں لکھیں۔",
//           isVoiceMessage
//         );
//         return res.sendStatus(200);
//       }

//       // Create priority request (AI analyzes the voice note)
//       const priorityRequest = await createPriorityRequest({
//         memberId: session.pending_member_id,
//         committeeId: session.pending_committee_id,
...
//       const fairnessLabel = priorityRequest.ai_fairness_score >= 80
//         ? "بہت زیادہ"
//         : priorityRequest.ai_fairness_score >= 60
//         ? "مناسب"
//         : "کم";
...
//         `وجہ: ${transcript_text}\n` +
//         `AI تجزیہ:\n` +
//         `- قسم: ${priorityRequest.ai_category}\n` +
//         `- فوریت: ${priorityRequest.ai_urgency}\n` +
//         `- ڈیڈلائن: ${priorityRequest.ai_deadline || "نہیں بتایا"}\n` +
//         `- انصاف اسکور: ${priorityRequest.ai_fairness_score}/100 (${fairnessLabel})\n\n` +
//         `AI تجویز: ${priorityRequest.ai_suggested ? "✅ ہاں — انصاف اسکور مناسب ہے" : "❌ نہیں — انصاف اسکور کم ہے"}\n\n` +
//         `${priorityRequest.ai_reasoning}\n\n` +
//         `اب کمیٹی کے ممبران ووٹ کریں گے۔ آپ کو نتیجے کی اطلاع دی جائے گی۔`,
//         isVoiceMessage
//       );

//       // Notify all other committee members about the new request
//       const { data: otherMembers } = await supabase
//         .from("members")
//         .select("phone, name")
//         .eq("committee_id", session.pending_committee_id)
//         .neq("id", session.pending_member_id);

//       const { data: requester } = await supabase
//         .from("members")
//         .select("name")
//         .eq("id", session.pending_member_id)
//         .single();

//       if (otherMembers && otherMembers.length > 0) {
//         const voteMsg =
//           `[Aitbaar] ${requester?.name || "ایک ممبر"} نے پرائیورٹی ادائیگی کی درخواست بھیجی ہے:\n\n` +
//           `"${transcript_text}"\n\n` +
//           `AI تجویز: ${priorityRequest.ai_suggested ? "✅ مناسب" : "❌ غیر مناسب"}\n` +
//           `انصاف اسکور: ${priorityRequest.ai_fairness_score}/100\n\n` +
//           `ووٹ کرنے کے لیے ڈیش بورڈ پر جائیں یا واٹس ایپ پر *Yes* یا *No* لکھیں。`;

//         for (const m of otherMembers) {
//           if (m.phone) {
//             sendWhatsAppMessage(m.phone, voteMsg).catch(() => {});
//           }
//         }
//       }
...
//     // ─────────────────────────────────────────────
//     // ALREADY REGISTERED — skip code check entirely
//     // ─────────────────────────────────────────────

//     if (isRegistered) {
//       const membership = memberships[0];
//       console.log("Registered member found:", membership.phone);
...
//   responseText =
//     `Shukriya ${membership.name || ""}! Aapki payment record ho gayi. ` +
...
//     sendWhatsAppMessage(
//       orgData.phone,
//       `[Aitbaar] ${membership.name || fromNumber} ne "${membership.committees?.name}" ki payment claim ki hai. Dashboard se Confirm ya Reject karein.`
//     ).catch(() => {});
//   }
//         // responseText =
//         //  `شکریہ ${membership.name || ""}! آپ کی ادائیگی ریکارڈ ہو گئی۔ ` +

//         //  `آرگنائزر تصدیق کرنے کے بعد ٹرسٹ اسکور اپ ڈیٹ ہوگا۔ ` +

//         // `ابھی آپ کا اسکور ${newScore} ہے۔`;
...
//         //   sendWhatsAppMessage(
//         //     orgData.phone,
//         //     `[Aitbaar] ${membership.name || fromNumber} نے "${membership.committees?.name}" کی ادائیگی کی تصدیق کی۔ ڈیش بورڈ سے تصدیق کریں۔`
//         //   ).catch(() => {});
//         // }

//       // Trust score query (just the number)
//       } else if (intent.intent === "trust_score_query") {
//         const score = await getTrustScore(membership.id, membership.committee_id);
//         responseText =
//           `Aapka trust score ${score}/100 hai. ` +
//           (score >= 80 ? "Bohat acha — aap ek reliable member hain!" :
//            score >= 50 ? "Theek hai — regular payments se score barhega." :
//            "Score thora kam hai — waqt pe payments karein.");

//       // Trust score explanation (detailed breakdown + why)
//       } else if (intent.intent === "trust_score_explanation") {
//         const { getFullExplanation, formatForWhatsApp } = require("../services/trustScoreExplainerService");
//         const fullExplanation = await getFullExplanation(membership.id, membership.committee_id);
//         const explanationMsg = formatForWhatsApp(fullExplanation);

//         await reply(fromNumber, explanationMsg, isVoiceMessage);
//         return res.sendStatus(200);

//       // Priority payout request — member wants early payout
//       } else if (intent.intent === "priority_request") {
//         // Set session to await the detailed voice note / text
//         await upsertSession(fromNumber, {
//           state: "awaiting_priority_voice",
//           pending_committee_id: membership.committee_id,
//           pending_member_id: membership.id,
//         });

//         await reply(
//           fromNumber,
//           `آپ نے پرائیورٹی ادائیگی کی درخواست کی ہے۔\n\n` +
//           `براہ کرم اپنی وجہ تفصیل سے بتائیں — آڈیو نوٹ یا ٹیکسٹ میں۔\n` +
//           `مثال: \"مجھے اس ماہ فیس بھرنی ہے\" یا \"گھر میں میڈیکل ایمرجنسی ہے\"\n\n` +
//           `AI آپ کی درخواست کا تجزیہ کرے گا اور کمیٹی کو بھیجے گا۔`,
//           isVoiceMessage
//         );
//         return res.sendStatus(200);
...
//       // General question
//       } else {
//         responseText = await generateResponse(transcript, membership);
//       }
...
//     // ─────────────────────────────────────────────
//     // NOT REGISTERED YET — check for committee code
//     // ─────────────────────────────────────────────

//     const codeMatch = transcript.match(COMMITTEE_CODE_PATTERN);

//     if (codeMatch) {
//       const rawCode = codeMatch[0].replace(/\s/g, "-").toUpperCase();
//       console.log(`Committee code detected: ${rawCode}`);

//       const committee = await findCommitteeByCode(rawCode);

//       if (!committee) {
//         await reply(fromNumber, "Yeh committee code sahi nahi hai. Apne organizer se dobara confirm kar lein.", isVoiceMessage);
//         return res.sendStatus(200);
//       }

//       const result = await registerMemberToCommittee(fromNumber, committee.id);

//       if (result.error) {
//         await reply(fromNumber, "Registration mein masla hua. Dobara try karein.", isVoiceMessage);
//         return res.sendStatus(200);
//       }

//       // Save session: waiting for name
//       await upsertSession(fromNumber, {
//         state: "awaiting_name",
//         pending_committee_id: committee.id,
//       });

//       await reply(
//         fromNumber,
//         `"${committee.name}" committee mein khush aamdeed! Aapka registration shuru ho gaya. Apna poora naam batayein.`,
//         isVoiceMessage
//       );
...
//     // ─────────────────────────────────────────────
//     // NEW USER, NO CODE — ask for committee code
//     // ─────────────────────────────────────────────

//     await reply(
//       fromNumber,
//     "السلامو علیکم! میں اعتبار ہوں۔ آپ کمیٹی میں رجسٹر ہونے کے لیے اپنا کمیٹی کوڈ بتائیں۔",
//       isVoiceMessage
//     );
