...
// NOTE: confirm this matches the env var name used in whatsappService.js
// (mediaService.js only needed WHATSAPP_ACCESS_TOKEN, not the phone number id,
// so this one hasn't been verified against your actual sender yet)
...
// ---------------------------------------------------------------------------
// Upload PDF to WhatsApp -> get media_id, then send as a document message
// ---------------------------------------------------------------------------
...
async function sendPdfDocument(to, mediaId, filename, caption) {
  return axios.post(
    `${GRAPH_BASE}/messages`,
    {
...
// ---------------------------------------------------------------------------
// Public function — call this right before your existing rulesAcceptancePrompt
// ---------------------------------------------------------------------------
async function sendMemberRulesPdf(phone, lang) {
  // "roman_urdu" has no separate script — reuse the English PDF for it
  const pdfLang = lang === "urdu" ? "urdu" : "english";
  const pdfPath = await generateRulesPdf(pdfLang);

  try {
    const mediaId = await uploadPdfToWhatsApp(pdfPath);
    const filename = pdfLang === "urdu" ? "Aitbaar_Member_Rules_UR.pdf" : "Aitbaar_Member_Rules_EN.pdf";
    const caption = pdfLang === "urdu" ? "براہ کرم اعتبار کمیٹی ممبر رولز پڑھیں۔" : "Please read the Aitbaar Committee Member Rules.";
    await sendPdfDocument(phone, mediaId, filename, caption);
  } catch (err) {
    // Never block registration if the PDF fails to send — the existing
    // text-based rulesAcceptancePrompt still goes out right after this.
    console.error("Error sending member rules PDF:", err.message);
  } finally {
    fs.unlink(pdfPath, () => {});
  }
}

module.exports = { sendMemberRulesPdf };